...
- Server-rendered forms and Tailwind CSS; no new dependency or migration
...
- `app/data/comments.ts` — public allowlisted list and trusted create operations
...
- Order public comments oldest first with id as a stable tie-breaker.
...
- Denial/abuse: cap content at 1,000 characters. Rate limiting, flagging, moderation, and deletion
...
- Validation tests cover trimming, blank input, maximum length, non-string input, and bounded safe
  redisplay values.
- Data tests cover stable ordering, report isolation, hidden-report exclusion, protected-field
  derivation, and output allowlisting.
- Controller tests cover guest visibility/login prompt, guest mutation redirect, CSRF rejection,
  hidden/missing `404`, linked `422` errors, escaped redisplay, successful `303`, and forged fields.
...
- Public output is stably ordered, escaped, and limited to comment text, date, and username.
